...
long unsigned int Frame::nNextId=0;     //下一个生成的帧的ID,这里是初始化类的静态成员变量
    ...
    mThDepth(thDepth)
{
	...
	// Step 3 对这个单目图像进行提取特征点（ 0-左图，1-右图 ）
    ExtractORB(0,imGray);
    ...
    // Step 4 用 OpenCV的矫正函数、内参对提取到的特征点进行矫正 
    UndistortKeyPoints();
    ...
    mThDepth(thDepth)
{
    ...
    mfLogScaleFactor = log(mfScaleFactor);  // 每层缩放因子的自然对数
