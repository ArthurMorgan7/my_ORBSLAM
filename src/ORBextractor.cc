...
ORBextractor::ORBextractor(int _nfeatures,		//指定要提取的特征点数目
    ...
    nfeatures(_nfeatures),      //设置这些参数
    scaleFactor(_scaleFactor), 
    nlevels(_nlevels),
    iniThFAST(_iniThFAST), 
    minThFAST(_minThFAST)
{
    // 调整图像金字塔vector以使得其符合设定的图像层数(cv::Mat)
    ...
    // 完成对每层特征点数量的分配
    //\\\\\\\\\\\\\\\\\\\\\\\/
	// 每层需要提取出来的特征点个数(int)，这个向量也要根据图像金字塔设定的层数进行调整
...
{ 
    ...
    for (int level = 0; level < nlevels; ++level)
    {
        vector<KeyPoint>& keypoints = allKeypoints[level]; // 第level层的特征点
...
void ORBextractor::ComputeKeyPointsOctTree(vector<vector<KeyPoint> >& allKeypoints)													
{
    ...
    // allKeypoints
    //  - 第一层：vector存储的是某图层里面的所有特征点
    //  - 第二层：存储的是整个图像金字塔中的所有图层里面的所有特征点
