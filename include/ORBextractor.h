...
public:
     ...
     * 
     * Notice: 注意这是一个不属于任何类的全局静态函数，static修饰符限定其只能够被本文件中的函数调用
     */
    void operator()(
        cv::InputArray image, 
        cv::InputArray mask, 
...
protected:
    ...
    std::vector<int> mnFeaturesPerLevel;		// 每层图像要提取的特征点数目 sum(mnFeaturesPerLevel) = nfeatures
