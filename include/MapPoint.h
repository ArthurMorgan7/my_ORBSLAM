...
public:
    ...
    // 获取当前地图点的被观测次数,单目+1，双目或RGB-D则+2
    int Observations();
...
public:
    ...
    const long int mnFirstKFid;         // 创建该 MapPoint的关键帧ID
    const long int mnFirstFrame;        // 创建该 MapPoint的帧ID（即每一关键帧有一个帧ID）
